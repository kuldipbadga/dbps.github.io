...
<?php
    ...
    if(isset($_POST['update']))
    {
        ...
        if($npass==$rpass)
        {
            $up=mysqli_query($con,"UPDATE `registration` SET `Password`='$npass' WHERE `Reg_id`='$id'");
            if($up)
            {
                echo "Password succesfully updated... ";
                ?><a href="Login.php">Back to Login page!</a><?php
            }
        }
    }
?>
	...
	<div class="login box box--big">
		...
		<form action="" method="post">
			<div class="agile-field-txt">
				...
				<input type="password" name="npass" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" placeholder="Enter new password" required="" />
			</div>
            <div class="agile-field-txt">
				<label>Re-Enter password</label>
				<input type="password" name="rpass" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" placeholder="Re-Enter password" required="" />
			</div>
