<?php
    ...
    if(isset($_POST["done"])) {
        ...
        if (file_exists($target_file3)) {
            $_SESSION['addslip_exist']="Sorry, file already exists.";
            $uploadOk = 0;
        }
        // Check file size
        ...
        if ($_FILES["addslip"]["size"] > 2000000) {
            $_SESSION['addslip_size']="Sorry, your file is too large. it must be less than 2MB";
            $uploadOk = 0;
        }
        ...
        if($imageFileType3 != "jpg" && $imageFileType3 != "png" && $imageFileType3 != "jpeg" ) {
            $_SESSION['addslip_type']="Sorry, only JPG, JPEG & PNG files are allowed.";
            $uploadOk = 0;
        }
        ...
        if ($mov0==true && $mov1==true && $mov2==true && $mov3==true) {
            $q="INSERT INTO `new_student_doc`(`id`, `reg_id`,`photo`,`icard`, `add_fee_re`, `add_slip`) VALUES ('','$id','$photo','$icard','$re','$slip')";
...
<body>
	...
	<div class="login box box--big">
		...
		<form action="" method="post" enctype="multipart/form-data">
			...
			<div class="agile-field-txt">
				<label> I-card</label>
				<input type="file" name="icard" required id="myInput"/s>
                <strong><font color=red>
			...
			<div class="agile-field-txt">
				<label> Addmission fee recipt</label>
				<input type="file" name="re" required id="myInput" />
                <font color=red >
            ...
            <div class="agile-field-txt">
				...
				<input type="file" name="addslip" required id="myInput" />
                <font color=red >
                <?php if(isset($_SESSION['addslip_exist']))
                        {
                            echo $_SESSION['addslip_exist'];
                            unset ($_SESSION['addslip_exist']);
                        }
                        if(isset($_SESSION['addslip_size']))
                        {
                            echo $_SESSION['addslip_size'];
                            unset ($_SESSION['addslip_size']);
                        }
                        if(isset($_SESSION['addslip_type']))
                        {
                            echo $_SESSION['addslip_type'];
                            unset ($_SESSION['addslip_type']);
                        }?></font>
            </div>
